...
public class Protocol {
	...
	public static boolean readPacketInternal() throws IOException {
		...
		if (opcode == ServerProt.AMILIOUS_NPC_MENU) {
			try {
				rt4.amilious.menu.NpcMenuOverrides.read(inboundBuffer, length);
			} catch (Throwable t) {
				t.printStackTrace();
			}
			opcode = -1;
		...
		if (opcode == ServerProt.AMILIOUS_GIGOS_HUD) {
			try {
				rt4.amilious.menu.GigosHud.read(inboundBuffer, length);
			} catch (Throwable t) {
				t.printStackTrace();
			}
			opcode = -1;
		...
		if (opcode == ServerProt.VARP_SMALL) {
			...
			// Let TutorialGuideReader decide if this iface matters
			AmiliousClient.onTutorialGuideText(id, iface, child, text != null ? text.toString() : "");
									...
									do {
										...
										if (priorityRequest == null) {
											...
											if (anInt3486 > 500) {
												...
												if ((modelId & 0x4) == 4) {
													Camera.cameraOffsetY += anInt4941;
												}
												if ((modelId & 0x2) == 2) {
											...
											if (MiniMap.anInt2252 > 500) {
												...
												modelId = (int) (Math.random() * 8.0D);
												if ((modelId & 0x1) == 1) {
													MiniMap.minimapOffsetY += MiniMap.minimapZoom;
												}
												if ((modelId & 0x2) == 2) {
													MiniMap.minimapOffsetX += MiniMap.minimapRotation;
												}
											}
											if (Camera.cameraOffsetX < -50) {
												Camera.anInt4229 = 2;
											}
											if (MiniMap.minimapOffsetY < -60) {
												MiniMap.minimapZoom = 2;
											}
											if (MiniMap.minimapOffsetX < -20) {
												MiniMap.minimapRotation = 1;
											}
	...
	public static ComponentPointer openInterface(@OriginalArg(1) int arg0, @OriginalArg(2) int arg1, @OriginalArg(3) int arg2) { // was method1148
		...
		//System.out.println("[dlg-open Protocol] iface=" + arg0);
