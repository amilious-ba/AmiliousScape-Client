...
public final class FogManager {
	...
	public static void applyLightPosition() {
		GlRenderer.api.glLightfv(GL2.GL_LIGHT0, GL2.GL_POSITION, light0Position, 0);
		GlRenderer.api.glLightfv(GL2.GL_LIGHT1, GL2.GL_POSITION, light1Position, 0);
	}
	...
	public static void setLightParams(@OriginalArg(0) int color, @OriginalArg(1) float ambientMod, @OriginalArg(2) float l0Diffuse, @OriginalArg(3) float l1Diffuse) {
		...
		GlRenderer.api.glLightModelfv(GL2.GL_LIGHT_MODEL_AMBIENT, lightModelAmbientParams, 0);
		@Pc(105) float[] light0Params = new float[]{l0Diffuse * red, l0Diffuse * green, l0Diffuse * blue, 1.0F};
		GlRenderer.api.glLightfv(GL2.GL_LIGHT0, GL2.GL_DIFFUSE, light0Params, 0);
		@Pc(139) float[] light1Params = new float[]{-l1Diffuse * red, -l1Diffuse * green, -l1Diffuse * blue, 1.0F};
		GlRenderer.api.glLightfv(GL2.GL_LIGHT1, GL2.GL_DIFFUSE, light1Params, 0);
	}
	...
	public static void setFogParams(@OriginalArg(0) int color, @OriginalArg(1) int offset) {
		...
		GlRenderer.api.glFogi(GL2.GL_FOG_MODE, GL2.GL_LINEAR);
		GlRenderer.api.glFogf(GL2.GL_FOG_DENSITY, 0.95F);
		GlRenderer.api.glHint(GL2.GL_FOG_HINT, GL2.GL_FASTEST);
		int fogEnd = GlobalConfig.VIEW_DISTANCE;
		@Pc(65) int fogStart = fogEnd - (int) (GlobalConfig.VIEW_FADE_DISTANCE * 2.0f) - offset;
		if (fogStart < 50) {
			fogStart = 50;
		}
		GlRenderer.api.glFogf(GL2.GL_FOG_START, (float) fogStart);
		GlRenderer.api.glFogf(GL2.GL_FOG_END, (float) fogEnd - GlobalConfig.VIEW_FADE_DISTANCE);
		GlRenderer.api.glFogfv(GL2.GL_FOG_COLOR, fogColor, 0);
	}
	...
	public static void setup() {
		GlRenderer.api.glColorMaterial(GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE);
		GlRenderer.api.glEnable(GL2.GL_COLOR_MATERIAL);
		@Pc(27) float[] light0Params = new float[]{0.0F, 0.0F, 0.0F, 1.0F};
		GlRenderer.api.glLightfv(GL2.GL_LIGHT0, GL2.GL_AMBIENT, light0Params, 0);
		GlRenderer.api.glEnable(GL2.GL_LIGHT0);
		@Pc(55) float[] light1Params = new float[]{0.0F, 0.0F, 0.0F, 1.0F};
		GlRenderer.api.glLightfv(GL2.GL_LIGHT1, GL2.GL_AMBIENT, light1Params, 0);
		GlRenderer.api.glEnable(GL2.GL_LIGHT1);
		lightColor = -1;
		fogColorRGB = -1;
		initDefaults();
	}
	...
	public static void setFogColor(@OriginalArg(0) float[] params) {
		...
		GlRenderer.api.glFogfv(GL2.GL_FOG_COLOR, params, 0);
	}
