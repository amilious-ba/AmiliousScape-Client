...
public class LightingManager {
	...
	private static void disableLight(@OriginalArg(0) int i) {
		if (enabledLights[i]) {
			...
			GlRenderer.api.glDisable(light);
		}
	}
	...
	public static void method2400() {
		...
		for (local3 = 0; local3 < 4; local3++) {
			...
			GlRenderer.api.glLightfv(local10, GL2.GL_AMBIENT, new float[]{0.0F, 0.0F, 0.0F, 1.0F}, 0);
			GlRenderer.api.glLightf(local10, GL2.GL_LINEAR_ATTENUATION, 0.0F);
			GlRenderer.api.glLightf(local10, GL2.GL_CONSTANT_ATTENUATION, 0.0F);
		}
		for (local3 = 0; local3 < 4; local3++) {
	...
	public static void method2402(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1, @OriginalArg(2) Tile[][][] arg2) {
		...
		GlRenderer.api.glDepthMask(false);
		GlRenderer.setLightingEnabled(false);
		GlRenderer.api.glBlendFunc(GL2.GL_DST_COLOR, GL2.GL_ONE);
		GlRenderer.api.glFogfv(GL2.GL_FOG_COLOR, new float[]{0.0F, 0.0F, 0.0F, 0.0F}, 0);
				...
				for (@Pc(112) int local112 = local84; local112 <= local92; local112++) {
					...
					for (@Pc(155) int local155 = local133; local155 <= local141; local155++) {
						...
						if (local66 < 0 || local160 != null && local160.aBoolean45) {
							...
							GlRenderer.api.glTexEnvfv(GL2.GL_TEXTURE_ENV, GL2.GL_TEXTURE_ENV_COLOR, new float[]{0.0F, 0.0F, 0.0F, local63.alpha}, 0);
		...
		GlRenderer.api.glBlendFunc(GL2.GL_SRC_ALPHA, GL2.GL_ONE_MINUS_SRC_ALPHA);
		GlRenderer.api.glDepthMask(true);
		GlRenderer.api.glFogfv(GL2.GL_FOG_COLOR, FogManager.fogColor, 0);
	...
	private static void method2403(@OriginalArg(0) int arg0, @OriginalArg(1) Light arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4) {
		...
		if (!enabledLights[arg0]) {
			GlRenderer.api.glEnable(local5);
			enabledLights[arg0] = true;
		}
		GlRenderer.api.glLightf(local5, GL2.GL_QUADRATIC_ATTENUATION, arg1.aFloat9);
		GlRenderer.api.glLightfv(local5, GL2.GL_DIFFUSE, arg1.diffuse, 0);
		aFloatArray17[0] = arg1.x - arg2;
		aFloatArray17[1] = arg1.y - arg3;
		aFloatArray17[2] = arg1.z - arg4;
		GlRenderer.api.glLightfv(local5, GL2.GL_POSITION, aFloatArray17, 0);
	}
