...
import rt4.render.texture.Texture;
import rt4.render.texture.TextureOp34;
...
public final class LiquidMaterialRenderer implements MaterialRenderer {
	...
	public LiquidMaterialRenderer() {
		if (this.anInt4831 < 0 && (GlRenderer.arbVertexProgramSupported && GlRenderer.maxTextureUnits >= 2)) {
			...
			GlRenderer.api.glGenProgramsARB(1, local19, 0);
	...
	public final void unbind() {
		if (this.anInt4831 >= 0) {
			GlRenderer.api.glCallList(this.anInt4831 + 1);
		}
	}
	...
	public final void bind() {
		...
		GlRenderer.api.glCallList(this.anInt4831);
		...
		if (!MaterialManager.allows3DTextureMapping) {
			GlRenderer.api.glBindTexture(GL2.GL_TEXTURE_2D, MaterialManager.anIntArray341[(int) ((float) (GlRenderer.anInt5323 * 64) * 0.005F) % 64]);
		}
		...
		for (@Pc(87) int local87 = 0; local87 < 64; local87++) {
			...
			GlRenderer.api.glProgramLocalParameter4fvARB(GL2.GL_VERTEX_PROGRAM_ARB, local87, this.aFloatBuffer1);
			local85 += 4;
		}
		if (MaterialManager.allows3DTextureMapping) {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 65, (float) GlRenderer.anInt5323 * 0.005F, 0.0F, 0.0F, 1.0F);
		} else {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 65, 0.0F, 0.0F, 0.0F, 1.0F);
		}
	...
	private void method3719() {
		this.anInt4831 = GlRenderer.api.glGenLists(2);
		GlRenderer.api.glNewList(this.anInt4831, GL2.GL_COMPILE);
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE1);
		if (MaterialManager.allows3DTextureMapping) {
		...
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE0);
		GlRenderer.api.glBindProgramARB(GL2.GL_VERTEX_PROGRAM_ARB, this.anInt4830);
		GlRenderer.api.glEnable(GL2.GL_VERTEX_PROGRAM_ARB);
		GlRenderer.api.glEndList();
		GlRenderer.api.glNewList(this.anInt4831 + 1, GL2.GL_COMPILE);
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE1);
		GlRenderer.api.glMatrixMode(GL2.GL_TEXTURE);
		...
		GlRenderer.api.glDisable(MaterialManager.allows3DTextureMapping ? GL2.GL_TEXTURE_3D : GL2.GL_TEXTURE_2D);
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE0);
		GlRenderer.api.glBindProgramARB(GL2.GL_VERTEX_PROGRAM_ARB, 0);
		GlRenderer.api.glDisable(GL2.GL_VERTEX_PROGRAM_ARB);
		GlRenderer.api.glDisable(GL2.GL_FRAGMENT_PROGRAM_ARB);
		GlRenderer.api.glEndList();
	}
	...
	public final void setArgument(@OriginalArg(0) int arg0) {
		...
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE1);
		if ((arg0 & 0x80) == 0) {
			GlRenderer.api.glEnable(MaterialManager.allows3DTextureMapping ? GL2.GL_TEXTURE_3D : GL2.GL_TEXTURE_2D);
		} else {
			GlRenderer.api.glDisable(MaterialManager.allows3DTextureMapping ? GL2.GL_TEXTURE_3D : GL2.GL_TEXTURE_2D);
		}
		GlRenderer.api.glActiveTexture(GL2.GL_TEXTURE0);
		if ((arg0 & 0x40) == 0) {
			GlRenderer.api.glGetFloatv(GL2.GL_LIGHT_MODEL_AMBIENT, aFloatArray24, 0);
			GlRenderer.api.glProgramLocalParameter4fvARB(GL2.GL_VERTEX_PROGRAM_ARB, 66, aFloatArray24, 0);
		} else {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 66, 1.0F, 1.0F, 1.0F, 1.0F);
		}
		@Pc(58) int local58 = arg0 & 0x3;
		if (local58 == 2) {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 64, 0.05F, 1.0F, 1.0F, 1.0F);
		} else if (local58 == 3) {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 64, 0.1F, 1.0F, 1.0F, 1.0F);
		} else {
			GlRenderer.api.glProgramLocalParameter4fARB(GL2.GL_VERTEX_PROGRAM_ARB, 64, 0.025F, 1.0F, 1.0F, 1.0F);
		}
	}
}
