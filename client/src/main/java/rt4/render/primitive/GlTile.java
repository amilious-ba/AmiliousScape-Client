...
import rt4.render.UnderwaterMaterialRenderer;
import rt4.scene.Tile;
import rt4.util.HashTable;
import rt4.util.IntNode;
...
public final class GlTile extends Node {
	...
	public final void method1944(@OriginalArg(0) Tile[][][] arg0, @OriginalArg(1) float arg1, @OriginalArg(2) boolean arg2) {
		...
		if (this.aClass155_3 == null) {
			...
			GlRenderer.api.glVertexPointer(3, GL2.GL_FLOAT, i, this.aByteBuffer3);
			this.aByteBuffer3.position(12);
			GlRenderer.api.glColorPointer(4, GL2.GL_UNSIGNED_BYTE, i, this.aByteBuffer3);
			if (Preferences.highDetailLighting) {
				this.aByteBuffer3.position(16);
				GlRenderer.api.glNormalPointer(GL2.GL_FLOAT, i, this.aByteBuffer3);
			}
			this.aByteBuffer3.position(28);
			GlRenderer.api.glTexCoordPointer(2, GL2.GL_FLOAT, i, this.aByteBuffer3);
			if (this.aBoolean139) {
				GlRenderer.api.glClientActiveTexture(UnderwaterMaterialRenderer.method4607());
				this.aByteBuffer3.position(36);
				GlRenderer.api.glTexCoordPointer(1, GL2.GL_FLOAT, i, this.aByteBuffer3);
				GlRenderer.api.glClientActiveTexture(GL2.GL_TEXTURE0);
			}
		} else {
			this.aClass155_3.bindArray();
			GlRenderer.api.glVertexPointer(3, GL2.GL_FLOAT, i, 0L);
			GlRenderer.api.glColorPointer(4, GL2.GL_UNSIGNED_BYTE, i, 12L);
			if (Preferences.highDetailLighting) {
				GlRenderer.api.glNormalPointer(GL2.GL_FLOAT, i, 16L);
			}
			GlRenderer.api.glTexCoordPointer(2, GL2.GL_FLOAT, i, 28L);
			if (this.aBoolean139) {
				GlRenderer.api.glClientActiveTexture(UnderwaterMaterialRenderer.method4607());
				GlRenderer.api.glTexCoordPointer(1, GL2.GL_FLOAT, i, 36L);
				GlRenderer.api.glClientActiveTexture(GL2.GL_TEXTURE0);
			}
		}
		if (GlRenderer.arbVboSupported) {
		...
		if (aClass3_Sub15_3.offset != 0) {
			...
			GlRenderer.api.glDrawElements(GL2.GL_TRIANGLES, aClass3_Sub15_3.offset / 4, GL2.GL_UNSIGNED_INT, aByteBuffer5);
		}
		...
		GlRenderer.api.glDrawElements(GL2.GL_TRIANGLES, aClass3_Sub15_2.offset / 4, GL2.GL_UNSIGNED_INT, aByteBuffer4);
