...
public final class GlVertexBufferObject {
	...
	public GlVertexBufferObject(@OriginalArg(0) boolean stream) {
		...
		GlRenderer.api.glGenBuffers(1, temp, 0);
		this.stream = stream;
		this.id = temp[0];
	...
	public final void updateArrayBuffer(@OriginalArg(0) ByteBuffer buffer) {
		if (buffer.limit() <= this.size) {
			GlRenderer.api.glBindBuffer(GL2.GL_ARRAY_BUFFER, this.id);
			GlRenderer.api.glBufferSubData(GL2.GL_ARRAY_BUFFER, 0, buffer.limit(), buffer);
		} else {
	...
	public final void setElementArrayBuffer(@OriginalArg(0) ByteBuffer buffer) {
		GlRenderer.api.glBindBuffer(GL2.GL_ELEMENT_ARRAY_BUFFER, this.id);
		GlRenderer.api.glBufferData(GL2.GL_ELEMENT_ARRAY_BUFFER, buffer.limit(), buffer, this.stream ? GL2.GL_STREAM_DRAW : GL2.GL_STATIC_DRAW);
	...
	public final void setArrayBuffer(@OriginalArg(0) ByteBuffer buffer) {
		GlRenderer.api.glBindBuffer(GL2.GL_ARRAY_BUFFER, this.id);
		GlRenderer.api.glBufferData(GL2.GL_ARRAY_BUFFER, buffer.limit(), buffer, this.stream ? GL2.GL_STREAM_DRAW : GL2.GL_STATIC_DRAW);
		GlCleaner.onCardGeometry += buffer.limit() - this.size;
		this.size = buffer.limit();
	}
}
