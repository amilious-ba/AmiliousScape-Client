...
import rt4.scene.PlayerAppearance;
import rt4.social.LocalizedText;
import rt4.social.PlayerList;
import rt4.ui.*;
import rt4.util.*;
import rt4.world.MapElement;
					...
					if (!component.if3 || local302 > local166 && local164 < local291) {
						...
						if (component.clientCode != 0) {
							...
							if (component.clientCode == 1338) {
								...
								memory = (int) Camera.yawTarget + MiniMap.minimapOffsetY & 0x7FF;
								local270 -= component.width / 2;
								color = MathUtils.sin[memory];
								cardMemory = MathUtils.cos[memory];
								color = (MiniMap.minimapOffsetX + 256) * color >> 8;
								cardMemory = (MiniMap.minimapOffsetX + 256) * cardMemory >> 8;
